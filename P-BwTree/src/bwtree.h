...
// To crash in the middle of splits, uncomment this
// #define CRASH_SPLIT

// To crash in the middle of merge, uncomment this
// #define CRASH_MERGE

// To crash in the middle of both split and merge, uncomment both the defines above.
/*
...
#define DUMMY_PTR_D (void*)0xfffffffe

#define NO_CRASH ((int)1)
#define CRASH_TYPE_SPLIT ((int) -1)
#define CRASH_TYPE_MERGE ((int) -2)
...
class BwTree : public BwTreeBase {
 ...
 public:
  ...
  inline NodeID GetNextNodeID() {
    ...
    //
    // In current implementation, this free list is initialized with empty list.
    ...
    if(ret_pair.first == false) {
      ...
      // automatically
      uint64_t ret = next_unused_node_id.fetch_add(1);
      clflush((char *)&next_unused_node_id, sizeof(std::atomic<NodeID>), false, true);
      return ret;
    } else {
  ...
  inline bool InstallNodeToReplace(NodeID node_id,
    ...
    NodeType type = node_p->GetType();
    NodeType ptype = prev_p->GetType();

    if (ptype == NodeType::InnerAbortType)
        goto remove_abort;
    ...
    switch(type) {
      ...
      default: {
        ...
        std::cout << "ERROR: Unknown node type = " << static_cast<int>(type) << std::endl;
        assert(false);
      }
    } // switch type
...
remove_abort:
  ...
  inline bool InstallRootNode(NodeID old_root_node_id,
    ...
    // Root node will be persisted by a call to InstallNode. 
    // This function only swaps the root pointer
    //std::cout << "Install ROOT node" << std::endl;
    bool ret = root_id.compare_exchange_strong(old_root_node_id,
                                           new_root_node_id);
    if (ret == true)
        clflush((char *) &root_id, sizeof(root_id), false, true);
  ...
  inline const BaseNode *GetNode(const NodeID node_id) {
	...
	//std::cout << __func__ << " Node id = " << node_id << std::endl;    
    assert(node_id < MAPPING_TABLE_SIZE);
...
retry_traverse:
    ...
    //std::cout << __func__ << ": Calling load_node_id " << start_node_id << std::endl;
    crash_type = LoadNodeID(start_node_id, context_p);

    // if crash, return dummyptr
    if (crash_type == CRASH_TYPE_SPLIT){
	//std::cout << __func__ << "Returning DUMMY_PTR" << std::endl;
	return (KeyValuePair *)DUMMY_PTR;
    }
    else if (crash_type == CRASH_TYPE_MERGE){
	//std::cout << __func__ << "Returning DUMMY_PTR_D" << std::endl;
	return (KeyValuePair *)DUMMY_PTR_D;
    }
    ...
    while(1) {
      ...
      //std::cout << __func__ << ": Calling child load_node_id " << child_node_id << std::endl;
      crash_type = LoadNodeID(child_node_id, context_p);
...
*/
  ...
  void JumpToLeftSibling(Context *context_p) {
    ...
    NodeSnapshot *snapshot_p = GetLatestNodeSnapshot(context_p);
    //std::cout << __func__ << " Asserted that there is a remove delta on node ID " << snapshot_p->node_id << std::endl;
    ...
    //std::cout << __func__ << " Asserted that its not on leftmost child" << std::endl;
   ...
   * Returns CRASH_TYPE_SPLIT or CRASH_TYPE_MERGE or NO_CRASH
   */
  int LoadNodeID(NodeID node_id, Context *context_p) {
	...
	// When a key insert requires a node split, the first step of SMO -> creating
	// the sibling node happens in that particular traverse function. However,
	// the second step of split completes in the subsequent insert in this function. 
    	//std::cout << __func__ << " Calling finish partial SMO" << std::endl;
	int status = FinishPartialSMO(context_p);
	// status is false only if we simulate crash
	if (status != NO_CRASH)
		return status;
    	...
    	//std::cout << __func__ << ":Adjust node size " << node_id << std::endl;
  ...
  void JumpToNodeID(NodeID node_id, Context *context_p) {
    ...
    //std::cout << __func__ << " calling finish SMO " << std::endl;
  ...
  inline void FinishPartialSMOReadOptimized(Context *context_p) {
    ...
    NodeSnapshot *snapshot_p = GetLatestNodeSnapshot(context_p);
    //std::cout << __func__ << " Before any op, lock is : " << context_p->abort_flag << std::endl;
before_switch:
	...
	/* The above said comment simply implies that readers retry in a loop
	 * until the ongoing SMO completes. However, if a crash occurs after
	 * posting the remove delta, but before posting the merge delta, 
	 * (note they are two atomic steps), then :
	 * A writer after the crash will see the remove delta and complete the
	 * node merge in FinishPartialSMO
	 * A reader must tolerate this state to make the removed node accessible
	 * To do so, we add the following behavior after crash in the reader:
	 * 1. We initialize the abort lock (common to readers and writers)
	 * 2. We use a try lock to identify if the reader has encountered 
	 * 	a crash state or if its a transient error.
	 * 	If the lock is set, then its transient, if not then its permanant
	 * 3. We perform the second step of merge, i.e. post a merge delta to the
	 * 	left sibling node. At this point, all the keys in the remove node are
	 * 	accessible through the left sibling. We need not complete the merge
	 * 	operation, as some other writer thread will observe the merge
	 *	delta and complete the operation. We do the minimal work required to
	 * 	tolerate inconsistency at the reader 
	 */
	...
	if (!context_p->abort_flag) {
		...
		//std::cout << __func__ << " Trying to get left sibling using JumpToLeftSibling with del node id = " << deleted_node_id << std::endl;
		...
		// If we are currently on leaf, just post leaf merge delta
        	if(left_snapshot_p->IsLeaf() == true) {
                	//std::cout << __func__ << ": MERGE STEP 1 : Leaf merge delta node " << merge_key << " : del " << deleted_node_id << std::endl;
          		ret = \
                	...
                	//std::cout << __func__ << ": MERGE STEP 1 : Inner merge delta node " << merge_key << " : del " << deleted_node_id << std::endl;
          		ret = \
        	...
        	if(ret == true) {
          		bwt_printf("Merge delta CAS succeeds. ""Continue to finish merge SMO\n");
			//std::cout << __func__ << " Merge CAS success" << std::endl;
			...
			//std::cout << __func__ << " Merge CAS fail. Retry" << std::endl;
          		context_p->abort_flag = true;
  ...
  inline void TakeNodeSnapshotReadOptimized(NodeID node_id,
                                            Context *context_p) {
    const BaseNode *node_p = GetNode(node_id);
    //std::cout << __func__ << " node id = " << node_id << ", mapping table = " << MAPPING_TABLE_SIZE << std::endl;
  ...
  inline void LoadNodeIDReadOptimized(NodeID node_id, Context *context_p) {
    ...
    TakeNodeSnapshot(node_id, context_p);
    //std::cout << __func__ << " : After snapshot, lock is : " << context_p->abort_flag << std::endl;
    FinishPartialSMOReadOptimized(context_p);
    //std::cout << __func__ << " : After partial SMO, lock is : " << context_p->abort_flag << std::endl;
...
retry_traverse:
    ...
    //std::cout << __func__ << ": LoadNodeID done" << std::endl;
    if(context_p->abort_flag == true) {
      bwt_printf("LoadNodeID aborted on loading root (RO)\n");

      //std::cout << __func__ << ": LoadNodeID aborted on loading root (RO)" << std::endl;
      goto abort_traverse;
    }
    ...
    while(1) {
      ...
      if(context_p->abort_flag == true) {
        ...
        //std::cout << __func__ << ": Navigate Inner Node abort (RO)" << std::endl;
      ...
      if(context_p->abort_flag == true) {
        ...
        //std::cout << __func__ << " LoadNodeID aborted (RO). ABORT for node " << child_node_id << std::endl;
	//std::cout << __func__ << " Lock value before exiting is : " << context_p->abort_flag << std::endl;
	//exit(1);
        goto abort_traverse;
      }
	  ...
	  //std::cout << __func__ << "NavigateLeafNode aborts (RO). ABORT" << std::endl;
          goto abort_traverse;
        }
...
abort_traverse:
   ...
   * 
   * Returns CRASH_TYPE_SPLIT or CRASH_TYPE_MERGE if interrupted due to crash
   * Else returns NO_CRASH
   */
  int FinishPartialSMO(Context *context_p) {
    // Note: If the top of the path list changes then this pointer
    // must also be updated
    NodeSnapshot *snapshot_p = GetLatestNodeSnapshot(context_p);
...
before_switch:
	...
	if (pid == 0){
		// unlock
		context_p->abort_flag = false;
		//std::cout << "\n Child process returned after 1st step of delete "<< deleted_node_id << std::endl;
		return CRASH_TYPE_MERGE;
	}
      ...
      case NodeType::LeafMergeType: {
        ...
        //std::cout << __func__ << ": MERGE STEP 2 : Parent inner delete node " << delete_item_p->first << " : " << delete_item_p->second << std::endl;
	  ...
	  //std::cout << __func__ << ": ROOT SPLIT" << std::endl;
          ...
          //std::cout << __func__ << ": SPLIT ROOT : Install root node " << snapshot_p->node_id  << " to " << new_root_id << std::endl;
	...
	// First consolidate parent node and find the right sep
    	...
    	if (pid == 0){
		 context_p->abort_flag = false;
         	//std::cout << "\nChild process returned after 1st step of split" << std::endl;
        	return CRASH_TYPE_SPLIT;
    	}
    	...
    	// parent process continue
         	...
         	waitpid(pid, &returnStatus, 0);
          	...
          	// Note: Even if this function aborts, since we return immediately
          	// so do not have to test abort_flag here
          	//std::cout << __func__ << ": SPLIT STEP 2 : Parent inner insert node " << insert_item_p->first << ":" << insert_item_p->second << std::endl;
  ...
  void AdjustNodeSize(Context *context_p) {
	...
	//std::cout << "Node size >= leaf upper threshold "<< node_size << ", Split " << LEAF_NODE_SIZE_UPPER_THRESHOLD << std::endl;
        bwt_printf("Node size >= leaf upper threshold. Split\n");

        // Note: This function takes this as argument since it will
        // do key comparison
	// This function returns the new split right sibling
	// with half the higher keys of the original node being split
	// This completes the first atomic step of node split
	// ############# ATOMIC 1 #################
    	//std::cout << "Getting split sibling" << std::endl;
    	...
    	//std::cout << "Install new node " << new_node_id << std::endl;
        InstallNewNode(new_node_id, new_leaf_node_p);

        // Then CAS split delta into current node's NodeID
    	//std::cout << __func__ << ": SPLIT STEP 1 : Install split sibling node " << node_id << "to" << new_node_id << std::endl;
        ...
        //std::cout << "Node size <= " << LEAF_NODE_SIZE_LOWER_THRESHOLD << "Remove" << std::endl;
    	...
    	//std::cout << __func__ << ": MERGE STEP 0 : Install leaf remove node " << node_id  << std::endl;
      ...
      if(node_size >= INNER_NODE_SIZE_UPPER_THRESHOLD) {
        ...
        // Then CAS split delta into current node's NodeID
    	//std::cout << __func__ << ": SPLIT STEP 1 : Install INNER split sibling node " << node_id << "to" << new_node_id << std::endl;
        bool ret = InstallNodeToReplace(node_id, split_node_p, node_p);
    	...
    	//std::cout << __func__ << ": MERGE STEP 0 : Install INNER remove node " << node_id << std::endl;
        bool ret = InstallNodeToReplace(node_id, remove_node_p, node_p);
        if(ret == true) {
  ...
  bool PostInnerMergeNode(const NodeSnapshot *snapshot_p,
    ...
    //std::cout << __func__ << " Inner merge node id = " << node_id << ", mapping table = " << MAPPING_TABLE_SIZE << std::endl;
  ...
  bool Insert(const KeyType &key, const ValueType &value) {
	...
	//std::cout << "Leaving epoch for key " << key << std::endl;
      ...
      if (item_p == DUMMY_PTR){
		//std::cout << __func__ << " Got DUMMY_PTR" << std::endl;
		//std::cout << "SKIPPED INSERTING DUE TO CRASH " << key << std::endl;
		return false;
      }
      else if (item_p == DUMMY_PTR_D){
		//std::cout << __func__ << " Got DUMMY_PTR_D" << std::endl;
		//std::cout << "SKIPPED INSERTING DUE TO CRASH IN FIXING SMO DURING MERGE " << key << std::endl;
		return false;
      }

      // We'll assume that the key insertion during which the crash occurs is retried
      // So we'll resume from this point for that key; otherwise we'll simply loop
  ...
  bool Delete(const KeyType &key, const ValueType &value) {
    ...
    while(1) {
      ...
      if (item_p == DUMMY_PTR_D){
		//std::cout << __func__ << " Got DUMMY_PTR_D" << std::endl;
		//std::cout << "SKIPPED DELETING DUE TO CRASH " << key << std::endl;
		return false;
      }
      if (item_p == DUMMY_PTR){
                //std::cout << __func__ << " Got DUMMY_PTR" << std::endl;
                //std::cout << "SKIPPED DELETING DUE TO CRASH IN FIXING SPLIT SMO " << key << std::endl;
                return false;
      }
